...
- 공급자별 입력·상태·비용은 adapter 경계에 가둬야 모델 전환 시 UI와 데이터 lifecycle을 안정적으로 보존할 수 있다.
...
- `CreateSongModal`을 추가해 Lyrics → Sound → Create 단계를 구현했다. 데스크톱은 좌측 세로 내비게이션, 모바일은 좌→우 가로 내비게이션을 제공한다.
- Lyrics는 직접 작성과 대화형 AI 작사 탭으로 나눴다. Sound는 Simple과 Advanced 모드로 나눠 실제 `GenerateRequest`에 연결했다.
- 데스크톱 모달은 단계별 콘텐츠 높이에 흔들리지 않는 고정 캔버스로 만들고, 좌측 단계 버튼은 모두 동일한 폭·높이로 고정했다.
- 기존 장르·무드·보컬·길이·언어·용도·프리셋을 실제 생성 요청으로 변환했다. 시안의 mock track·credit·타이머·선택 곡은 사용하지 않는다.
- 실제 폴링 상태에 맞춰 모달 안에서 추정 진행·완료·환불 실패 UI를 표시하며, 모달을 닫아도 생성과 optimistic row 폴링은 계속된다.
...
# RESULT: 제목 기반 앨범 커버 prediction 병렬화 - 2026-07-13

## Background

음악 생성 완료 뒤 장시간 썸네일 작업이 pending으로 고착될 수 있어, 제목 기반 cover prediction을 음악 prediction과 병렬 시작하도록 전환했다.

## Implementation

prediction ID를 metadata에 저장하고, 이후 폴링에서 Storage URL/key와 완료/실패 상태를 반영하도록 분리했다.

## Verification Matrix

129 tests, build, lint, diff check를 통과했다.

## Lessons

서버리스 환경의 긴 외부 작업은 prediction ID를 저장하고 짧은 폴링에서 결과를 반영하는 구조가 적합하다.

---

# RESULT: Gemini 오디오 가사 싱크 기능 삭제 - 2026-07-13

## Background
MVP 범위를 넘는 Gemini 기반 오디오 가사 정렬 기능을 삭제하되, 가사 텍스트 표시와 API 호출 없는 approximate 하이라이트는 유지했다.

## Implementation
`lib/lyrics/sync.ts`·`ensureLyricsSync.ts`와 테스트를 삭제하고, generate/music/reconcile 라우트 및 워크스페이스의 관련 상태·폴링을 제거했다. `lyrics_sync_*`/`lyrics_lrc` metadata를 제거하는 마이그레이션도 적용했다.

## Verification Matrix
전체 단위 테스트, 타입체크, build, lint를 통과했다. 기존 곡 49건에서 `lyrics_sync_status`를 제거했고 가사 텍스트는 유지됨을 DB 조회로 확인했다.

## Lessons
기능 삭제는 코드뿐 아니라 영속 metadata와 클라이언트 폴링 경로까지 함께 정리해야 한다.
