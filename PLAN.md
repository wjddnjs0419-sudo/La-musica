...
## Done
- [Done] Pricing update — Viral 35곡 + Free 가입 1곡 지급 (2026-06-17) - `lib/credits.ts` viral-pack 50→35, 신규 마이그레이션으로 `fulfill_polar_credit_order` RPC 맵 동기화 + 멱등 `grant_free_credit()` 추가, OAuth 콜백에서 신규 유저 무료 1곡 지급, dead Stripe 마이그레이션 삭제. Trial 생략·가격 동일(Polar 제품 불필요). build/lint/마이그레이션 적용/prod 배포 완료. See RESULT.md.
